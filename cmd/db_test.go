...
func Test_DatabaseCommand_db_setup_for_network(t *testing.T) {
	...
	t.Run("run for all tenants", func(t *testing.T) {
		...
		assert.Len(t, actualAssets, 6)
		assert.Equal(t, assets.EURCAssetCode, actualAssets[0].Code)
		assert.Equal(t, assets.EURCAssetIssuerPubnet, actualAssets[0].Issuer)
		assert.Equal(t, assets.POSTERAssetCode, actualAssets[1].Code)
		assert.Equal(t, assets.MerchAssetIssuerPubnet, actualAssets[1].Issuer)
		assert.Equal(t, assets.STICKERAssetCode, actualAssets[2].Code)
		assert.Equal(t, assets.MerchAssetIssuerPubnet, actualAssets[2].Issuer)
		assert.Equal(t, assets.SWAGAssetCode, actualAssets[3].Code)
		assert.Equal(t, assets.SWAGAssetIssuerPubnet, actualAssets[3].Issuer)
		assert.Equal(t, assets.USDCAssetCode, actualAssets[4].Code)
		assert.Equal(t, assets.USDCAssetIssuerPubnet, actualAssets[4].Issuer)
		assert.Equal(t, assets.XLMAssetCode, actualAssets[5].Code)
		assert.Empty(t, actualAssets[5].Issuer)
