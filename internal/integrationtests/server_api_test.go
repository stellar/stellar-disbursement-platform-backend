...
func Test_RegisterEmbeddedWallet(t *testing.T) {
	httpClientMock := httpclientMocks.HTTPClientMock{}
...
func Test_GetEmbeddedWallet(t *testing.T) {
	httpClientMock := httpclientMocks.HTTPClientMock{}
	...
	t.Run("error calling httpClient.Do", func(t *testing.T) {
		httpClientMock.On("Do", mock.AnythingOfType("*http.Request")).Return(nil, fmt.Errorf("error calling the request")).Once()
		wallet, err := sa.GetEmbeddedWallet(ctx, credentialID)
		require.EqualError(t, err, "making request to GET /embedded-wallets/{credentialID}: error calling the request")
		assert.Nil(t, wallet)
	...
	t.Run("error wallet not found", func(t *testing.T) {
		errorResponse := `{"error": "Wallet not found"}`
		response := &http.Response{
			Body:       io.NopCloser(strings.NewReader(errorResponse)),
		...
		wallet, err := sa.GetEmbeddedWallet(ctx, credentialID)
		require.EqualError(t, err, "error getting embedded wallet (statusCode=404)")
		assert.Nil(t, wallet)
	...
	t.Run("successfully getting embedded wallet with pending status", func(t *testing.T) {
		walletResponse := `{
			"status": "PENDING"
		}`
	...
	t.Run("successfully getting embedded wallet with deployed contract", func(t *testing.T) {
		walletResponse := `{
			"status": "SUCCESS",
		...
		wallet, err := sa.GetEmbeddedWallet(ctx, credentialID)
		require.NoError(t, err)
		assert.Equal(t, data.SuccessWalletStatus, wallet.Status)
		assert.Equal(t, "CDLZFC3SYJYDZT7K67VZ75HPJVIEUVNIXF47ZG2FB2RMQQVU2HHGCYSC", wallet.ContractAddress)
