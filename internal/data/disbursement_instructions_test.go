...
func Test_DisbursementInstructionModel_ProcessAll(t *testing.T) {
	...
	t.Run("success - process instructions with contract address", func(t *testing.T) {
		defer cleanup()
		dbTx := testutils.BeginTxWithRollback(t, ctx, dbConnectionPool)
		...
		// Verify Receiver
		receivers, err := di.receiverModel.GetByContacts(ctx, dbTx, instructions[0].Phone)
		require.NoError(t, err)
		require.Len(t, receivers, 1)

		// Verify Receiver Wallet has contract address and NO memo
		receiverWallets, err := di.receiverWalletModel.GetWithReceiverIDs(ctx, dbTx, []string{receivers[0].ID})
		require.NoError(t, err)
		require.Len(t, receiverWallets, 1)
		assert.Equal(t, wallet.ID, receiverWallets[0].Wallet.ID)
		assert.Equal(t, contractAddress, receiverWallets[0].StellarAddress)
		assert.Empty(t, receiverWallets[0].StellarMemo)
		assert.Empty(t, receiverWallets[0].StellarMemoType)
		assert.Equal(t, RegisteredReceiversWalletStatus, receiverWallets[0].Status)

		// Verify Payment
		actualPayments := GetPaymentsByDisbursementID(t, ctx, dbTx, knownWalletDisbursement.ID)
		assert.Len(t, actualPayments, 1)
		assert.Contains(t, actualPayments, instructions[0].Amount)
	...
	t.Run("success - mixed G-address and C-address instructions", func(t *testing.T) {
		defer cleanup()
		dbTx := testutils.BeginTxWithRollback(t, ctx, dbConnectionPool)

		gAddress := "GCVL44LFV3BFI627ABY3YRITFBRJVXUQVPLXQ3LISMI5UVKS5LHWTPT7"
		instructions := []*DisbursementInstruction{
			{
				...
				Amount:            "100.01",
				ID:                "1",
				Phone:             "+380-12-345-681",
			},
			{
				WalletAddress: contractAddress,
				Amount:        "100.02",
				ID:            "2",
				Phone:         "+380-12-345-682",
			},
		}
		...
		receiverWallets, err := di.receiverWalletModel.GetWithReceiverIDs(ctx, dbTx, []string{receivers[0].ID, receivers[1].ID})
		require.NoError(t, err)
		require.Len(t, receiverWallets, 2)

		for _, rw := range receiverWallets {
			assert.Equal(t, wallet.ID, rw.Wallet.ID)
			assert.Equal(t, RegisteredReceiversWalletStatus, rw.Status)

			switch rw.StellarAddress {
			case gAddress:
				// G-address should have memo
				assert.Equal(t, "123456789", rw.StellarMemo)
				assert.NotEmpty(t, rw.StellarMemoType)
			case contractAddress:
				// Contract address should have NO memo
				assert.Empty(t, rw.StellarMemo)
				assert.Empty(t, rw.StellarMemoType)
			default:
				t.Errorf("unexpected stellar address: %s", rw.StellarAddress)
			}
		}
	})

	t.Run("failure - contract address with memo is rejected", func(t *testing.T) {
		defer cleanup()
		dbTx := testutils.BeginTxWithRollback(t, ctx, dbConnectionPool)
		...
		require.Error(t, err)
		assert.ErrorIs(t, err, ErrMemosNotSupportedForContractAddresses)
	})
}
