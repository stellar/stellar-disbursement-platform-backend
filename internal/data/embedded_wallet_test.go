...
func Test_EmbeddedWalletStatus_Validate(t *testing.T) {
	testCases := []struct {
		name        string
		status      EmbeddedWalletStatus
		expectError bool
	}{
		{name: "valid PENDING status", status: PendingWalletStatus, expectError: false},
		{name: "valid PROCESSING status", status: ProcessingWalletStatus, expectError: false},
		{name: "valid SUCCESS status", status: SuccessWalletStatus, expectError: false},
		{name: "valid FAILED status", status: FailedWalletStatus, expectError: false},
		{name: "valid lowercase pending", status: "pending", expectError: false},
		{name: "valid mixed case Pending", status: "Pending", expectError: false},
		{name: "invalid status", status: "INVALID", expectError: true},
		{name: "empty status", status: "", expectError: true},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expectError {
				require.Error(t, err)
				assert.Contains(t, err.Error(), "invalid embedded wallet status")
			} else {
...
func Test_EmbeddedWalletModel_GetByCredentialID(t *testing.T) {
	dbt := dbtest.Open(t)
	defer dbt.Close()
	...
	t.Run("returns error when wallet does not exist", func(t *testing.T) {
		wallet, err := embeddedWalletModel.GetByCredentialID(ctx, dbConnectionPool, "non_existent_credential")
		require.Error(t, err)
		assert.Equal(t, ErrRecordNotFound, err)
		assert.Nil(t, wallet)
	})

	t.Run("returns wallet when it exists by credential ID", func(t *testing.T) {
		expectedWasmHash := "abcdef123456"
		expectedCredentialID := "test-credential-id-123"
		expectedPublicKey := "04" + strings.Repeat("ab", 64)
		createdWallet := CreateEmbeddedWalletFixture(t, ctx, dbConnectionPool, "", expectedWasmHash, "", expectedCredentialID, expectedPublicKey, SuccessWalletStatus)

		wallet, err := embeddedWalletModel.GetByCredentialID(ctx, dbConnectionPool, expectedCredentialID)
		require.NoError(t, err)
		require.NotNil(t, wallet)

		assert.Equal(t, createdWallet.Token, wallet.Token)
		assert.Equal(t, expectedWasmHash, wallet.WasmHash)
		assert.Equal(t, expectedCredentialID, wallet.CredentialID)
		assert.Equal(t, expectedPublicKey, wallet.PublicKey)
		assert.Equal(t, SuccessWalletStatus, wallet.WalletStatus)
	})
}

func Test_EmbeddedWalletModel_Insert(t *testing.T) {
	dbt := dbtest.Open(t)
	defer dbt.Close()
	...
	t.Run("returns error for invalid insert", func(t *testing.T) {
		insert := EmbeddedWalletInsert{
			Token:        "",
			WasmHash:     "abcdef123456",
			WalletStatus: PendingWalletStatus,
		}
		wallet, err := embeddedWalletModel.Insert(ctx, dbConnectionPool, insert)
		require.Error(t, err)
		assert.Contains(t, err.Error(), "token cannot be empty")
		assert.Nil(t, wallet)
	})

	t.Run("successfully inserts a new embedded wallet", func(t *testing.T) {
		insert := EmbeddedWalletInsert{
			Token:                "test-token-insert-123",
			WasmHash:             "abcdef123456",
			WalletStatus:         PendingWalletStatus,
		...
		assert.Equal(t, insert.Token, wallet.Token)
		assert.Equal(t, insert.WasmHash, wallet.WasmHash)
		assert.Equal(t, insert.WalletStatus, wallet.WalletStatus)
		assert.True(t, wallet.RequiresVerification)
		assert.NotNil(t, wallet.CreatedAt)
		assert.NotNil(t, wallet.UpdatedAt)
	})

	t.Run("returns error for duplicate token", func(t *testing.T) {
		insert := EmbeddedWalletInsert{
		...
		_, err := embeddedWalletModel.Insert(ctx, dbConnectionPool, insert)
		require.NoError(t, err)
