...
func (tw *TransactionWorker) handleFailedTransaction(ctx context.Context, txJob *TxJob, hTxResp horizon.Transaction, txErr utils.TransactionError) error {
	...
	shouldMarkAsError := txErr.ShouldMarkAsError()
	shouldReportToCrashTracker := txErr.ShouldReportToCrashTracker()
	errorType := txErr.GetErrorType()

	log.Ctx(ctx).Errorf("🔴 Error processing job (%s): %v", errorType, txErr)

	defer tw.txHandler.MonitorTransactionProcessingFailed(ctx, txJob, tw.jobUUID, isRetryable, errorMsg)
	...
	// Adjust processing limits based on error type
	txErr.AdjustLimits(tw.txProcessingLimiter)

	// Handle terminal errors that should mark transaction as ERROR
	if shouldMarkAsError {
		if markErr := tw.markTransactionAsError(ctx, txJob, txErr, errorMsg); markErr != nil {
			return markErr
		}

		// Report terminal errors to crash tracker
		if shouldReportToCrashTracker {
			tw.crashTrackerClient.LogAndReportErrors(ctx, txErr, fmt.Sprintf("%s transaction error - cannot be retried", strings.ToLower(errorType)))
		}
	} else {
		// For retryable errors, check if transaction needs rebuilding
		if isRetryable && tw.txHandler.RequiresRebuildOnRetry() {
			if prepareErr := tw.prepareTransactionForReprocessing(ctx, txJob); prepareErr != nil {
				return fmt.Errorf("preparing transaction for rebuild: %w", prepareErr)
			}
		}
...
// prepareTransactionForReprocessing clears transaction metadata to force rebuilding on next attempt
func (tw *TransactionWorker) prepareTransactionForReprocessing(ctx context.Context, txJob *TxJob) error {
	_, err := tw.txModel.PrepareTransactionForReprocessing(ctx, tw.dbConnectionPool, txJob.Transaction.ID)
	if err != nil {
		return fmt.Errorf("preparing transaction for reprocessing: %w", err)
	}
	return nil
}
