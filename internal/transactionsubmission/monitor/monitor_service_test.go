...
func Test_TSSMonitorService_LogAndMonitorTransaction(t *testing.T) {
	...
	testCases := []struct {
		...
		// payment_id correlation: when the SDP Transaction.ExternalID is set, the log entry must
		// carry it as `payment_id` so operators can correlate a TSS log back to the SDP payment.
		{
			name:      "monitor payment sets payment_id from external_id",
			metricTag: sdpMonitor.PaymentTransactionSuccessfulTag,
			txMetadata: TxMetadata{
				EventID:              "test_event_id_7",
				TransactionEventType: sdpMonitor.PaymentProcessingSuccessfulLabel,
				SrcChannelAcc:        srcChannelAcc,
			},
			txModel: store.Transaction{
				ID:         txID,
				ExternalID: "payment-abc-123",
				TenantID:   tenantID,
				CreatedAt:  &time,
				UpdatedAt:  &time,
				Payment: store.Payment{
			...
			fieldsMap: map[string]interface{}{
				...
				"app_version":     tssMonitorSvc.Version,
				"channel_account": srcChannelAcc,
				"event_id":        "test_event_id_7",
				"event_type":      sdpMonitor.PaymentProcessingSuccessfulLabel,
				"git_commit_hash": tssMonitorSvc.GitCommitHash,
