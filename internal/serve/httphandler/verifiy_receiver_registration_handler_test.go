...
func Test_VerifyReceiverRegistrationHandler_producePaymentsReadyToPayEvent(t *testing.T) {
	...
	t.Run("doesn't return error when the msg is nil", func(t *testing.T) {
		getEntries := log.DefaultLogger.StartTest(log.WarnLevel)

		handler := VerifyReceiverRegistrationHandler{}
		err := handler.producePaymentsReadyToPayEvent(ctx, nil)
		require.NoError(t, err)

		entries := getEntries()
		require.Len(t, entries, 1)
		assert.Equal(t, "message is nil, not producing event", entries[0].Message)
	})

	t.Run("logs when couldn't write message because EventProducer is nil", func(t *testing.T) {
		getEntries := log.DefaultLogger.StartTest(log.ErrorLevel)

		handler := VerifyReceiverRegistrationHandler{EventProducer: nil}
		err := handler.producePaymentsReadyToPayEvent(ctx, &msg)
		require.NoError(t, err)

		entries := getEntries()
		require.Len(t, entries, 1)
		expectedLog := fmt.Sprintf("event producer is nil, could not publish message %+v", msg)
	...
	t.Run("returns error when EventProducer fails writing a message", func(t *testing.T) {
		...
		handler := VerifyReceiverRegistrationHandler{EventProducer: &mockEventProducer}

		mockEventProducer.
			On("WriteMessages", ctx, []events.Message{msg}).
			Return(errors.New("unexpected error")).
			Once()

		err := handler.producePaymentsReadyToPayEvent(ctx, &msg)
		expectedErrMessage := fmt.Sprintf("writing message %+v on event producer: unexpected error", msg)
		assert.EqualError(t, err, expectedErrMessage)

		mockEventProducer.AssertExpectations(t)
	})

	t.Run("🎉 successfully produces the event", func(t *testing.T) {
		mockEventProducer := events.MockProducer{}
		handler := VerifyReceiverRegistrationHandler{EventProducer: &mockEventProducer}
