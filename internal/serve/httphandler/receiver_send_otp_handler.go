...
func (h ReceiverSendOTPHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		httperror.BadRequest("invalid request body", err, nil).WithErrorCode(ErrorCode400_0).Render(w)
		return
	}
	...
	if !h.ReCAPTCHADisabled {
		...
		if tokenErr != nil {
			httperror.InternalError(ctx, "Cannot validate reCAPTCHA token", tokenErr, nil).WithErrorCode(ErrorCode500_5).Render(w)
			return
		}
		if !isValid {
			log.Ctx(ctx).Errorf("reCAPTCHA token is invalid")
			httperror.BadRequest("reCAPTCHA token is invalid", nil, nil).WithErrorCode(ErrorCode400_1).Render(w)
			return
		}
	}
...
func (h ReceiverSendOTPHandler) handleOTPForReceiver(
	...
	if err != nil {
		return placeholderVerificationField, httperror.InternalError(ctx, "Cannot generate OTP for receiver wallet", err, nil).WithErrorCode(ErrorCode500_7)
	}
	...
	if err != nil && !errors.Is(err, data.ErrRecordNotFound) {
		return placeholderVerificationField, httperror.InternalError(ctx, "Cannot update OTP for receiver wallet", err, nil).WithErrorCode(ErrorCode500_8)
	}
	...
	if err != nil {
		...
		return placeholderVerificationField, httperror.InternalError(ctx, "Failed to send OTP message, reason: "+err.Error(), err, nil).WithErrorCode(ErrorCode500_9)
	}
