...
func (w *WebAuthnService) FinishPasskeyAuthentication(ctx context.Context, request *http.Request) (*data.EmbeddedWallet, error) {
	...
	getUserForCredential := func(rawID, userHandle []byte) (webauthn.User, error) {
		...
		embeddedWallet, err := w.sdpModels.EmbeddedWallets.GetByCredentialID(ctx, w.sdpModels.DBConnectionPool, credentialID)
		if err != nil {
			return nil, fmt.Errorf("getting embedded wallet by credential ID: %w", err)
		}
