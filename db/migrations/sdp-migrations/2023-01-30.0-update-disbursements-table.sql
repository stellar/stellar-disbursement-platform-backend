...
ALTER TABLE disbursements RENAME COLUMN requested_at TO created_at;

-- columns name & id
UPDATE disbursements SET name = id;
ALTER TABLE disbursements
...
-- column updated_at
CREATE TRIGGER refresh_disbursement_updated_at BEFORE UPDATE ON disbursements FOR EACH ROW EXECUTE PROCEDURE update_at_refresh();

-- column status_history
UPDATE disbursements SET status_history = ARRAY[create_disbursement_status_history(created_at::TIMESTAMP, disbursement_status('STARTED'), NULL)];
UPDATE disbursements SET status_history = array_prepend(create_disbursement_status_history(NOW(), disbursement_status('COMPLETED'), NULL), status_history) WHERE status = disbursement_status('COMPLETED');


-- +migrate Down
DROP TRIGGER refresh_disbursement_updated_at ON disbursements;
...
ALTER TABLE disbursements RENAME COLUMN created_at TO requested_at;
