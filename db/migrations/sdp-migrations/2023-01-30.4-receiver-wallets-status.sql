...
-- COLUMN: status
UPDATE receiver_wallets rwOriginal
    SET status = (
    ...
    FROM receiver_wallets rw LEFT JOIN receivers r ON rw.receiver_id = r.id
...
-- TABLE: receiver
ALTER TABLE receivers DROP COLUMN status;


-- +migrate Down

-- TABLE: receiver
ALTER TABLE receivers ADD COLUMN status VARCHAR(32);
